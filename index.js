...
owner = "𝗠𝗮𝘅_ᴅᴇᴠᴇʟᴏᴩᴍᴇɴᴛꜱᵈᵉᵛ"

client.on('ready', () => {
  console.log(`${client.user.username} is Online`)
setInterval(async () => {
const statuses = [`MaxGamer Server`, `| Made by ${owner}`]
   client.user.setActivity(statuses[Math.floor(Math.random() * statuses.length)], { type: "STREAMING", url: "https://www.twitch.tv/"})
}, 10000)
});
...
// command stuff
  let messageArray = message.content.split(" "),
    ...
    prefix = (process.env.prefix); // Add Prefix in the ENV
...
client.on("message", async message => {
  if (message.channel.type === "dm") {
    ...
    const chat = new Chat({ name: "Manager", gender: "male", developer_name: "MaxGamer", user: "806810037459746846", language: "en" }); //put a random id here
